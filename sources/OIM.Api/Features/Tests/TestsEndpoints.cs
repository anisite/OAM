...
using OIM.Workflow.Core.Engine;
using OIM.Workflow.Core.Yaml;

namespace OIM.Api.Features.Tests;
...
public static class TestsEndpoints
{
    public static IEndpointRouteBuilder MapTests(this IEndpointRouteBuilder app)
    {
            ...
            string nom) =>
        {
            ...
            OIM.Domain.Entities.InstanceWorkflow instance;
            try
            {
                // Exécution synchrone directe — bypass queue et claim pour éviter la race condition
                instance = await moteur.DemarrerEtExecuterAsync(definitionId, parsed.InputJson, correlationId);
            }
            finally
            {
