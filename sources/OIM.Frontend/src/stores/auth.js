import { defineStore } from 'pinia'
import { ref } from 'vue'

export const useAuthStore = defineStore('auth', () => {
  const token = ref(localStorage.getItem('oim_token'))
  const utilisateur = ref(localStorage.getItem('oim_utilisateur'))

  function estExpire() {
    if (!token.value) return true
    try {
      const payload = JSON.parse(atob(token.value.split('.')[1]))
      // Renouveler 60 secondes avant l'expiration réelle
      return payload.exp * 1000 < Date.now() + 60_000
    } catch {
      return true
    }
  }

  async function initialiser() {
    if (!token.value || estExpire()) {
      await obtenirToken()
    }
  }

  async function obtenirToken() {
    try {
      // En dev, utiliser dev-token (pas de NTLM) ; en prod, NTLM via /api/auth/token
      const url = import.meta.env.DEV ? '/api/auth/dev-token' : '/api/auth/token'
      const response = await fetch(url, { credentials: 'include' })
      if (response.ok) {
        const data = await response.json()
        token.value = data.token
        utilisateur.value = data.utilisateur
        localStorage.setItem('oim_token', data.token)
        localStorage.setItem('oim_utilisateur', data.utilisateur)
      }
    } catch (err) {
      console.error('Erreur authentification:', err)
    }
  }

  function deconnecter() {
    token.value = null
    utilisateur.value = null
    localStorage.removeItem('oim_token')
    localStorage.removeItem('oim_utilisateur')
  }

  function headers() {
    return token.value ? { Authorization: `Bearer ${token.value}` } : {}
  }

  return { token, utilisateur, initialiser, obtenirToken, deconnecter, headers }
})
